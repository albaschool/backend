...
import { createDefaultOfAcademy, createDefaultOfConvenienceAndMart, createDefaultOfEntertainment, createDefaultOfRestaurantAndCafe, createDefaultOfSalesAndService } from "@/services/educationPage.service";
...
export const createStore = async (req: Request, res: Response) => {
  ...
  let eduResult;
  if(type === StoreType.RetaurantAndCafe)
    eduResult = createDefaultOfRestaurantAndCafe(storeId);
  else if (type === StoreType.ConvenienceAndMart)
    eduResult = createDefaultOfConvenienceAndMart(storeId);
  else if (type === StoreType.SalesAndService)
    eduResult = createDefaultOfSalesAndService(storeId);
  else if (type === StoreType.Entertainment)
    eduResult = createDefaultOfEntertainment(storeId);
  else if (type == StoreType.Academy)
    eduResult = createDefaultOfAcademy(storeId);
  if(!eduResult)
    throw new Error("Failed to create default education page");
